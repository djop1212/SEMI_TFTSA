 ...
 * 
 * // 업로드된 파일 저장 폴더 지정 String savePath =
 ...
 * "전송파일 저장 실패"); return "common/error"; } }// 업로드된 파일 있음
 * //student.setOriginal_filename(originFileName); }// 첨부 파일 있음
 * 
 ...
 * // 오프너 새로고침 str += "self.close();"; // 창닫기 str += "</script>";
 * out.print(str); return null; }else { model.addAttribute("message",
 * "학생 프로필 추가 실패..."); return "common/error"; } }
 * 
 * // 선생님 프로필 추가 페이지로 이동
 * 
 ...
 * 
 * // 과목번호 subdata에 저장, tutor 테이블 subname for(int i=0;i<array.length;i++) {
 ...
 * } catch (Exception e) { e.printStackTrace(); model.addAttribute("message",
 * "전송파일 저장 실패"); return "common/error"; } } // 업로드된 파일 있음 //
 * tutor.setOriginal_filename(originFileName); } // 첨부 파일 있음
 * 
 ...
 * tutor.setTime(stime+", "+etime); tutor.setArea(city+" "+country);
 * 
 * String []array = arr_sub_no.split(","); String sub_name=""; // 과목번호 subdata에
 * 저장, tutor 테이블 subname if (mypageService.deleteSubData(member.getUser_no()) >
 ...
 * 
 * // 새로운 암호가 전송이 왔다면 String user_pwd = member.getUser_pwd().trim(); if
 * (user_pwd != null && user_pwd.length() > 0) { // 기존 암호와 다른 값이면 암호화 처리를 해라 if
 * (!bcryptPasswordEncoder.matches(user_pwd, originUserpwd)) {
 * member.setUser_pwd(bcryptPasswordEncoder.encode(user_pwd)); } } else { // 새로운
 * 암호값이 전송오지 않았다면 원래 암호를 기록 member.setUser_pwd(originUserpwd); }
 * 
 ...
 * "redirect:myPage.do?user_id=" + member.getUser_id(); } else {
 * model.addAttribute("message", member.getUser_id() + "회원정보 수정 실패.."); return
 * "common/error"; } }
 * 
 * // 채팅 목록 조회 컨트롤러
 * 
 ...
 * "등록된 채팅목록 정보가 없습니다."); return "common/error"; } } }
 */